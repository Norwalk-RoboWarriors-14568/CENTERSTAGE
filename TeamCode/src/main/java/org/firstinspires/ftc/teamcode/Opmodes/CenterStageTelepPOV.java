...
public class CenterStageTeleOp extends OpMode {
    ...
    @Override
    public void loop(){
        if (abs(gamepad1.left_stick_x)> 0.20) {
        ...
        if (mecanumDriveMode) {
                ...
                fr.setPower((gamepad1.left_stick_y + mecanumStrafe) / 1.5);
                br.setPower((gamepad1.left_stick_y + -mecanumStrafe) / 1.5);
        } else {
                drive(gamepad1.left_stick_y * 0.8, gamepad1.left_stick_y * 0.8 );
        }
        if (abs(gamepad1.right_stick_x) > 0.2){
            fl.setPower(gamepad1.right_stick_y / 1.5);
            bl.setPower(gamepad1.right_stick_y / 1.5);
            fr.setPower(-gamepad1.right_stick_y / 1.5);
            br.setPower(-gamepad1.right_stick_y / 1.5);
        }
    }
    @Override
    public void stop() {
